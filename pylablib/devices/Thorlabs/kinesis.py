...
class BasicKinesisDevice(comm_backend.ICommBackendWrapper):
    ...
    def __init__(self, conn, timeout=3.0, is_rack_system=False):
        defaults = {
            "serial": {"baudrate": 115200, "rtscts": True},
            "ft232": {"baudrate": 115200, "rtscts": True},
        }
        instr = comm_backend.new_backend(
            conn,
            backend=("auto", "ft232"),
            term_write=b"",
            term_read=b"",
            timeout=timeout,
            defaults=defaults,
            reraise_error=ThorlabsBackendError,
        )
    ...
    def _cycle_rts(instr):
        ...
        if be == "ft232":
            ...
            warnings.warn(
                "could not cycle RTS with backend '{}'; some devices might not work properly".format(
                    be
                )
            )
    ...
    def send_comm(self, messageID, param1=0x00, param2=0x00, source=0x01, dest="host"):
        ...
        """
        msg = struct.pack(
            "<HBBBB", messageID, param1, param2, self._make_dest(dest), source
        )
    ...
    def send_comm_data(self, messageID, data, source=0x01, dest="host"):
        ...
        """
        msg = struct.pack(
            "<HHBB", messageID, len(data), self._make_dest(dest) | 0x80, source
        )
        self.instr.write(msg + data)

    CommShort = collections.namedtuple(
        "CommShort", ["messageID", "param1", "param2", "source", "dest"]
    )
    CommData = collections.namedtuple(
        "CommData", ["messageID", "data", "source", "dest"]
    )
        ...
        while True:
            ...
            if messageID in self._bg_msg_counters and messageID != expected_id:
                ...
                if expected_id is not None and messageID != expected_id:
                    raise ThorlabsError(
                        "unexpected command received: expected 0x{:04x}, got 0x{:04x}".format(
    ...
    def query(
        ...
        if flush:
            self.flush_comm()
        self.send_comm(
            messageID, param1=param1, param2=param2, source=source, dest=dest
        )
    ...
    def get_device_info(self, dest="host"):
        ...
        if not self._model_match(model_no, port_model_no):
            warnings.warn(
                "model number {} doesn't match the device ID prefix {}({})".format(
                    model_no, port, port_model_no
                )
            )
        ...
        return TDeviceInfo(
            serial_no, model_no, fw_ver, hw_type, hw_ver, mod_state, nchannels, notes
        )
...
TVelocityParams = collections.namedtuple(
    "TVelocityParams", ["min_velocity", "acceleration", "max_velocity"]
)
TJogParams = collections.namedtuple(
    "TJogParams",
    ["mode", "step_size", "min_velocity", "acceleration", "max_velocity", "stop_mode"],
)
TGenMoveParams = collections.namedtuple("TGenMoveParams", ["backlash_distance"])
THomeParams = collections.namedtuple(
    "THomeParams", ["home_direction", "limit_switch", "velocity", "offset_distance"]
)
TPolCtlParams = collections.namedtuple(
    "TPolCtlParams", ["velocity", "home_position", "jog1", "jog2", "jog3"]
)
TLimitSwitchParams = collections.namedtuple(
    "TLimitSwitchParams",
    [
        "hw_kind_cw",
        "hw_kind_ccw",
...
TKCubeTrigIOParams = collections.namedtuple(
    "TKCubeTrigIOParams", ["trig1_mode", "trig1_pol", "trig2_mode", "trig2_pol"]
)
TKCubeTrigPosParams = collections.namedtuple(
    "TKCubeTrigPosParams",
    [
        "start_fw",
        "step_fw",
...
TPZMotorDriveParams = collections.namedtuple(
    "TPZMotorDriveParams", ["max_voltage", "velocity", "acceleration"]
)
TPZMotorJogParams = collections.namedtuple(
    "TPZMotorJogParams",
    ["mode", "step_size_fw", "step_size_bk", "velocity", "acceleration"],
)
...
class KinesisDevice(IMultiaxisStage, BasicKinesisDevice):
    ...
    def __init__(self, conn, timeout=3.0, default_channel=1, is_rack_system=False):
        super().__init__(
            conn,
            timeout=timeout,
            is_rack_system=is_rack_system,
            default_axis=default_channel,
        )
    ...
    def _get_status_n(self, channel=None):
        ...
        """
        data = self.query(
            self._status_comm, self._make_channel(channel), dest=("channel", channel)
        ).data
    ...
    status_bits = [
        ...
        (1 << 16, "interlock"),
        (1 << 17, "overtemp"),
        (1 << 18, "volt_supply_fault"),
        (1 << 19, "commutation_error"),
        (1 << 20, "digio1"),
        (1 << 21, "digio2"),
        (1 << 22, "digio3"),
        (1 << 23, "digio4"),
    ...
    def _wait_for_status(
        self, status, enabled, channel=None, timeout=None, period=0.05
    ):
        """
        ...
        for s in status:
            funcargparse.check_parameter_range(
                s, "status", [s for _, s in self.status_bits]
            )
    ...
    def _get_position(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x0490 if self._status_comm == 0x0490 else 0x0411,
            self._make_channel(channel),
    ...
    def _set_position_reference(self, position=0, channel=None, scale=True):
        ...
        self.send_comm_data(
            ...
            struct.pack(
                "<Hi",
                self._make_channel(channel),
                self._p2d(position, "p", scale=scale),
            ),
            dest=("channel", channel),
        )
    ...
    def _move_by(self, distance=1, channel=None, scale=True):
        ...
        if self._move_by_mode == "move_to":
            self._move_to(
                self._get_position(channel=channel, scale=scale) + distance,
                channel=channel,
                scale=scale,
            )
        else:
            self.send_comm_data(
                ...
                struct.pack(
                    "<Hi",
                    self._make_channel(channel),
                    self._p2d(distance, "p", scale=scale),
                ),
                dest=("channel", channel),
            )
    ...
    def _wait_move(self, channel=None, timeout=None):
        ...
        return self._wait_for_status(
            self._moving_status, False, channel=channel, timeout=timeout
        )
    ...
    def _wait_for_stop(self, channel=None, timeout=None):
        ...
        return self._wait_for_status(
            self._moving_status + ["homing"], False, channel=channel, timeout=timeout
        )
    ...
    def _get_velocity_parameters(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x0414, self._make_channel(channel), dest=("channel", channel)
        ).data
    ...
    def _setup_velocity(
        ...
        min_velocity = (
            current_parameters.min_velocity
            if min_velocity is None
            else self._p2d(min_velocity, "v", scale=scale)
        )
        acceleration = (
            current_parameters.acceleration
            if acceleration is None
            else self._p2d(acceleration, "a", scale=scale)
        )
        max_velocity = (
            current_parameters.max_velocity
            if max_velocity is None
            else self._p2d(max_velocity, "v", scale=scale)
        )
        data = struct.pack(
            "<Hiii",
            self._make_channel(channel),
    ...
    _p_stop_mode = interface.EnumParameterClass(
        "jog_stop_mode", {"immediate": 1, "profiled": 2}
    )

    @muxchannel
    @interface.use_parameters(
        _returns=["jog_mode", None, None, None, None, "jog_stop_mode"]
    )
    def _get_jog_parameters(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x0417, self._make_channel(channel), dest=("channel", channel)
        ).data
        mode, step_size, min_velocity, acceleration, max_velocity, stop_mode = (
            struct.unpack("<HiiiiH", data[2:22])
        )
    ...
    def _setup_jog(
        ...
        step_size = (
            ...
            else self._p2d(step_size, "p", scale=scale)
        )
        min_velocity = (
            current_parameters.min_velocity
            if min_velocity is None
    ...
    def _get_adc_inputs(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x042B, self._make_channel(channel), dest=("channel", channel)
        ).data
    ...
    def _get_gen_move_parameters(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x043B, self._make_channel(channel), dest=("channel", channel)
        ).data
    ...
    def _setup_gen_move(self, backlash_distance=None, channel=None, scale=True):
        ...
        backlash_distance = (
            current_parameters.backlash_distance
            if backlash_distance is None
            else self._p2d(backlash_distance, "p", scale=scale)
        )
    ...
    _p_home_direction = interface.EnumParameterClass(
        "home_direction", {"forward": 1, "reverse": 2}
    )
    _p_limit_switch = interface.EnumParameterClass(
        "limit_switch", {"reverse": 1, "forward": 4}
    )
    ...
    def _get_homing_parameters(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x0441, self._make_channel(channel), dest=("channel", channel)
        ).data
        home_direction, limit_switch, velocity, offset_distance = struct.unpack(
            "<HHii", data[2:14]
        )
    ...
    @muxchannel(
        mux_argnames=["home_direction", "limit_switch", "velocity", "offset_distance"]
    )
    @interface.use_parameters
    def _setup_homing(
        self,
        ...
        If ``scale==True``, assume that the specified values are in the physical units (see class description); otherwise, assume it is in the device internal units.
        """
        current_parameters = self._wap._get_homing_parameters(
            channel=channel, scale=False
        )
        home_direction = (
            current_parameters.home_direction
            if home_direction is None
            else home_direction
        )
        limit_switch = (
            current_parameters.limit_switch if limit_switch is None else limit_switch
        )
        velocity = (
            current_parameters.velocity
            if velocity is None
            else self._p2d(velocity, "v", scale=scale)
        )
        offset_distance = (
            current_parameters.offset_distance
            if offset_distance is None
            else self._p2d(offset_distance, "p", scale=scale)
        )
        data = struct.pack(
            "<HHHii",
            self._make_channel(channel),
    ...
    _p_hw_limit_kind = interface.EnumParameterClass(
        "hw_limit_kind",
        {
            "ignore": 1,
            "make": 2,
    ...
    @muxchannel
    @interface.use_parameters(
        _returns=["hw_limit_kind", "hw_limit_kind", None, None, None, "sw_limit_kind"]
    )
    def _get_limit_switch_parameters(self, channel=None, scale=True):
        ...
        """
        data = self.query(
            0x0424, self._make_channel(channel), dest=("channel", channel)
        ).data
        hw_kind_cw, hw_kind_ccw, sw_position_cw, sw_position_ccw, sw_kind = (
            struct.unpack("<HHiiH", data[2:16])
        )
        swapped_cw = bool(hw_kind_cw & 0x80)
        swapped_ccw = bool(hw_kind_ccw & 0x80)
        if swapped_cw != swapped_ccw:
            warnings.warn(
                "swapped bit is different for the two axes; assume that the switches are not swapped"
            )
        sw_position_cw = self._d2p(sw_position_cw, "p", scale=scale)
        sw_position_ccw = self._d2p(sw_position_ccw, "p", scale=scale)
        return TLimitSwitchParams(
            hw_kind_cw & 0x7F,
            hw_kind_ccw & 0x7F,
            swapped_cw,
            sw_position_cw,
    ...
    @interface.use_parameters(
        hw_kind_cw="hw_limit_kind", hw_kind_ccw="hw_limit_kind", sw_kind="sw_limit_kind"
    )
    def _setup_limit_switch(
        self,
        hw_kind_cw=None,
        hw_kind_ccw=None,
        hw_swapped=None,
        sw_position_cw=None,
        sw_position_ccw=None,
        sw_kind=None,
        channel=None,
        scale=True,
    ):
        ...
        """
        current_parameters = self._wap._get_limit_switch_parameters(
            channel=channel, scale=False
        )
        hw_kind_cw = current_parameters.hw_kind_cw if hw_kind_cw is None else hw_kind_cw
        hw_kind_ccw = (
            current_parameters.hw_kind_ccw if hw_kind_ccw is None else hw_kind_ccw
        )
        ...
        sw_position_cw = (
            current_parameters.sw_position_cw
            if sw_position_cw is None
            else self._p2d(sw_position_cw, "p", scale=scale)
        )
        sw_position_ccw = (
            current_parameters.sw_position_ccw
            if sw_position_ccw is None
            else self._p2d(sw_position_ccw, "p", scale=scale)
        )
    ...
    _p_kcube_trigio_mode = interface.EnumParameterClass(
        "kcube_trigio_mode",
        {
            "off": 0x00,
            "in_gpio": 0x01,
    ...
    @interface.use_parameters(
        _returns=["kcube_trigio_mode", None, "kcube_trigio_mode", None]
    )
    def _get_kcube_trigio_parameters(self):
        ...
        trig1_mode, trig1_pol, trig2_mode, trig2_pol = struct.unpack(
            "<HHHH", data[2:10]
        )
        return TKCubeTrigIOParams(
            trig1_mode, bool(trig1_pol), trig2_mode, bool(trig2_pol)
        )

    @interface.use_parameters(
        trig1_mode="kcube_trigio_mode", trig2_mode="kcube_trigio_mode"
    )
    def _setup_kcube_trigio(
        self, trig1_mode=None, trig1_pol=None, trig2_mode=None, trig2_pol=None
    ):
        """
    ...
    def _get_kcube_trigpos_parameters(self, scale=True):
        ...
        start_fw, step_fw, num_fw, start_bk, step_bk, num_bk, width, ncycles = (
            struct.unpack("<iiiiiiii", data[2:34])
        )
        ...
        return TKCubeTrigPosParams(
            start_fw, step_fw, num_fw, start_bk, step_bk, num_bk, width / 1e6, ncycles
        )

    def _setup_kcube_trigpos(
        self,
        start_fw=None,
        step_fw=None,
        num_fw=None,
        ...
        start_fw = (
            current_parameters.start_fw
            if start_fw is None
            else self._p2d(start_fw, "p", scale=scale)
        )
        step_fw = (
            current_parameters.step_fw
            if step_fw is None
            else self._p2d(step_fw, "p", scale=scale)
        )
        num_fw = current_parameters.num_fw if num_fw is None else num_fw
        start_bk = (
            current_parameters.start_bk
            if start_bk is None
            else self._p2d(start_bk, "p", scale=scale)
        )
        step_bk = (
            current_parameters.step_bk
            if step_bk is None
            else self._p2d(step_bk, "p", scale=scale)
        )
    ...
    def _setup_polctl(
        ...
        home_position = (
            current_parameters.home_position
            if home_position is None
            else self._p2d(home_position, "p", scale=scale)
        )
        jog1 = (
            current_parameters.jog1
            if jog1 is None
            else self._p2d(jog1, "p", scale=scale)
        )
        jog2 = (
            current_parameters.jog2
            if jog2 is None
            else self._p2d(jog2, "p", scale=scale)
        )
        jog3 = (
            current_parameters.jog3
            if jog3 is None
            else self._p2d(jog3, "p", scale=scale)
        )
        data = b"\x00\x00" + struct.pack(
            "<HHHHH", velocity, home_position, jog1, jog2, jog3
        )
        self.send_comm_data(0x0530, data)
        return self._get_polctl_parameters(scale=scale)

    _p_channel_id = interface.EnumParameterClass(
        "channel_id", {1: 0x01, 2: 0x02, 3: 0x04, 4: 0x08}
    )
    ...
    def _is_channel_enabled(
        self, channel=None
    ):  # seems to not work for most devices when tested
    ...
    @interface.use_parameters(channel="channel_id")
    def _enable_channel(
        self, enabled=True, channel=None
    ):  # seems to not work for most devices when tested
        """Enable or disable the given channel"""
        self.send_comm(
            0x0210,
            self._make_channel(channel),
            0x01 if enabled else 0x02,
            dest=("channel", channel),
        )
    ...
    def _pzmot_get_status_n(self, channel=None):
        ...
        status = [
            struct.unpack("<I", data[i * 14 + 10 : i * 14 + 14])[0] for i in range(4)
        ]
    ...
    def _pzmot_req(self, subid, channel):
        ...
        if rchannel != channel:
            raise RuntimeError(
                "unexpected channel in the reply: expected 0x{:02x}, got 0x{:02x}".format(
                    channel, rchannel
                )
            )
        if rsubid != subid:
            raise RuntimeError(
                "unexpected submessage ID in the reply: expected 0x{:02x}, got 0x{:02x}".format(
                    subid, rsubid
                )
            )
        return data[4:]
    ...
    def _pzmot_setup_drive(
        self, max_voltage=None, velocity=None, acceleration=None, channel=None
    ):
        """
        ...
        max_voltage = (
            current_parameters.max_voltage if max_voltage is None else max_voltage
        )
        velocity = current_parameters.velocity if velocity is None else velocity
        acceleration = (
            current_parameters.acceleration if acceleration is None else acceleration
        )
    ...
    def _pzmot_setup_jog(
        ...
        mode = current_parameters.mode if mode is None else mode
        step_size_fw = (
            current_parameters.step_size_fw if step_size_fw is None else step_size_fw
        )
        step_size_bk = (
            current_parameters.step_size_bk if step_size_bk is None else step_size_bk
        )
        velocity = current_parameters.velocity if velocity is None else velocity
        acceleration = (
            current_parameters.acceleration if acceleration is None else acceleration
        ...
        if self._pzmot_get_kind() == "TIM":
            ...
            data = struct.pack(
                "<HIIII", mode, step_size_fw, step_size_bk, velocity, acceleration
            )
            self._pzmot_set(0x2D, channel, data)
        return self._pzmot_get_jog_parameters(channel)

    _p_pzmot_channel_enabled = interface.EnumParameterClass(
        "pzmot_channel_enabled",
        {
            False: 0,
            None: 0,
            1: 1,
    ...
    def _pzmot_move_to(self, position, auto_enable=True, channel=None):
        ...
        self._pzmot_autoenable(
            {0x01: 1, 0x02: 2, 0x04: 3, 0x08: 4}[channel], auto_enable
        )
    ...
    def _pzmot_move_by(self, distance=1, auto_enable=True, channel=None):
        ...
        self._pzmot_move_to(
            self._pzmot_get_position(channel) + distance,
    ...
    def _pzmot_wait_for_status(
        self, status, enabled, timeout=None, period=0.05, channel=None
    ):
        return self._wait_for_status(
            status, enabled, channel=channel, timeout=timeout, period=period
        )
    ...
    _p_pzctl_control_mode = interface.EnumParameterClass(
        "pzctl_control_mode",
        {
            "open_loop": 1,
            "closed_loop": 2,
    ...
    def _pzctl_get_control_mode(self, channel=None):
        ...
        if not self._model.startswith("BPC30"):
            return "open_loop"  # Default for non-BPC3xx devices
        msg = self.query(0x0641, channel)
        return msg.param2

    @muxchannel
    @interface.use_parameters(mode="pzctl_control_mode")
    def _pzctl_set_control_mode(self, mode, channel=None):
        """Set piezo controller position control mode"""
        self.send_comm(0x0640, channel, mode)
        return self._pzctl_get_control_mode(channel)

    def _pzctl_position_u2d(self, pos):
        """Convert position from 0-100% to device units (0-32767)"""
        return max(0, min(32767, int(pos / 100.0 * 32767)))

    def _pzctl_position_d2u(self, pos):
    ...
    @muxchannel
    def _pzctl_set_zero_position(self, channel=None):
    ...
    def _pzctl_get_status_bits(
        ...
        Returns tuple (voltage, position, status_bits) where:
        - voltage: current output voltage (converted to specified units)
        - position: current position reading (converted to specified units)
    ...
    @muxchannel
    def _pzctl_get_max_output_voltage(self, channel=None):
        """Get maximum output voltage setting"""
        data = self.query(0x0681, channel).data
        channel_id, voltage_steps, flags = struct.unpack("<HHH", data[:6])
        return voltage_steps / 10.0  # Convert from 0.1V steps to volts

    @muxchannel
    def _pzctl_set_max_output_voltage(self, max_voltage, channel=None):
        """Set maximum output voltage (0-150V depending on hardware)"""
        voltage_steps = int(max_voltage * 10)
        voltage_steps = max(0, min(voltage_steps, 1500))  # Clamp to valid range
        data = struct.pack("<HHH", channel, voltage_steps, 0)
        self.send_comm_data(0x0680, data)
        return self._pzctl_get_max_output_voltage(channel)

    @muxchannel
    def _pzctl_get_slew_rates(self, channel=None):
        """Get current slew rates (open_loop_rate, closed_loop_rate)"""
        data = self.query(0x0684, channel).data
        channel_id, open_loop_rate, closed_loop_rate = struct.unpack("<HHH", data[:6])
        return (open_loop_rate, closed_loop_rate)

    @muxchannel
    def _pzctl_set_slew_rates(
        self, open_loop_rate=None, closed_loop_rate=None, channel=None
    ):
        """Set slew rates for voltage changes (0-32767)"""
        if open_loop_rate is None and closed_loop_rate is None:
            return self._pzctl_get_slew_rates(channel)
        current_rates = self._pzctl_get_slew_rates(channel)
        open_loop_rate = current_rates[0] if open_loop_rate is None else open_loop_rate
        closed_loop_rate = (
            current_rates[1] if closed_loop_rate is None else closed_loop_rate
        )
    ...
    @muxchannel
    def _pzctl_factory_reset(self, channel=None):
        """Restore factory settings from EEPROM"""
...
class MFF(KinesisDevice):
    ...
    _p_io_oper_mode = interface.EnumParameterClass(
        "io_oper_mode",
        {"in_toggle": 1, "in_position": 2, "out_position": 3, "out_motion": 4},
    )
    _p_io_sig_mode = interface.EnumParameterClass(
        "io_sig_mode",
        {
            "in_button": 0x01,
            "in_voltage": 0x02,
    ...
    def get_flipper_parameters(self, channel=None):
        ...
        (
            ...
            io2_sig_mode,
            io2_pulse_width,
        ) = struct.unpack("<iiHHiHHi", data[2:26])
        return TFlipperParameters(
            transit_time * 1e-3,
            io1_oper_mode,
            io1_sig_mode,
    ...
    @interface.use_parameters(
        io1_oper_mode="io_oper_mode",
        io1_sig_mode="io_sig_mode",
        io2_oper_mode="io_oper_mode",
        io2_sig_mode="io_sig_mode",
    )
    def setup_flipper(
        self,
        transit_time=None,
        io1_oper_mode=None,
        io1_sig_mode=None,
        io1_pulse_width=None,
        io2_oper_mode=None,
        io2_sig_mode=None,
        io2_pulse_width=None,
        channel=None,
    ):
        """
        ...
        transit_time = (
            current_parameters.transit_time if transit_time is None else transit_time
        )
        transit_time = int(transit_time * 1e3)
        transit_time_adc = int(1e7 * transit_time**-1.591)
        io1_oper_mode = (
            current_parameters.io1_oper_mode if io1_oper_mode is None else io1_oper_mode
        )
        io1_sig_mode = (
            current_parameters.io1_sig_mode if io1_sig_mode is None else io1_sig_mode
        )
        io1_pulse_width = (
            current_parameters.io1_pulse_width
            if io1_pulse_width is None
            else io1_pulse_width
        )
        io1_pulse_width = int(io1_pulse_width * 1e3)
        io2_oper_mode = (
            current_parameters.io2_oper_mode if io2_oper_mode is None else io2_oper_mode
        )
        io2_sig_mode = (
            current_parameters.io2_sig_mode if io2_sig_mode is None else io2_sig_mode
        )
        io2_pulse_width = (
            current_parameters.io2_pulse_width
            if io2_pulse_width is None
            else io2_pulse_width
        )
...
class KinesisMotor(KinesisDevice):
    ...
    def __init__(self, conn, scale="step", default_channel=1, is_rack_system=False):
        super().__init__(
            conn, default_channel=default_channel, is_rack_system=is_rack_system
        )
        ...
        if not self._model.startswith("MPC"):
            self._add_settings_variable(
                "velocity_parameters", self.get_velocity_parameters, self.setup_velocity
            )
            self._add_settings_variable(
                "jog_parameters", self.get_jog_parameters, self.setup_jog
            )
            self._add_settings_variable(
                "homing_parameters", self.get_homing_parameters, self.setup_homing
            )
            self._add_settings_variable(
                "gen_move_parameters", self.get_gen_move_parameters, self.setup_gen_move
            )
            self._add_settings_variable(
                "limit_switch_parameters",
                self.get_limit_switch_parameters,
                self.setup_limit_switch,
            )
        else:
            self._add_settings_variable(
                "polctl_parameter", self.get_polctl_parameters, self.setup_polctl
            )
        if self._model in ["KDC101", "KST101", "KBD101"]:
            if self._model != "KST101":
                self._add_settings_variable(
                    "kcube_trigio_parameters",
                    self.get_kcube_trigio_parameters,
                    self.setup_kcube_trigio,
                )
            self._add_settings_variable(
                "kcube_trigpos_parameters",
                self.get_kcube_trigpos_parameters,
                self.setup_kcube_trigpos,
            )
        with self._close_on_error():
    ...
    def _get_step_scale(self, stage):
        ...
        if (
            self._model in ["TST001", "MST601"]
            or self._model.startswith("BSC00")
            or self._model.startswith("BSC10")
        ):
        ...
        if (
            self._model in ["TST101", "KST101", "MST602", "K10CR1"]
            or self._model.startswith("BSC20")
            or self._model.startswith("SSC20")
        ):
            if stage.startswith("ZST"):
        ...
        if self._model.startswith("MPC"):
            return 1370 / 170, "deg"
        warnings.warn(
            "can't recognize the stage name {}; assuming step scale of 1".format(stage)
        )
        return 1, "step"
    ...
    def _calculate_scale(self, scale):
        ...
        if (
            self._model in ["TBD001", "KBD101"]
            or self._model.startswith("BBD10")
            or self._model.startswith("BBD20")
        ):
            time_conv = 102.4e-6
            return (ssc, ssc * time_conv * 2**16, ssc * time_conv**2 * 2**16), units
        if (
        ...
        warnings.warn(
            "can't recognize motor model {}; setting all scales to internal units".format(
                self._model
            )
        )
...
class KinesisPiezoMotor(KinesisDevice):
    ...
    def __init__(self, conn, default_channel=1, is_rack_system=False):
        ...
        self._add_status_variable(
            "enabled_channels", self.get_enabled_channels, self.enable_channels
        )
        ...
        self._add_settings_variable(
            "drive_parameters",
            lambda: self.get_drive_parameters(channel="all"),
            lambda v: self.setup_drive(v, channel="all"),
        )
        self._add_settings_variable(
            "jog_parameters",
            lambda: self.get_jog_parameters(channel="all"),
            lambda v: self.setup_jog(v, channel="all"),
        )
        with self._close_on_error():
    ...
    def _autodetect_axes(self):
        if self._model in ["TIM001", "KIM001"]:
            ...
            warnings.warn(
                "unexpected piezo motor model: {}; assuming one axis".format(
                    self._model
                )
            )
            self._update_axes([1])
...
class KinesisPiezoController(KinesisDevice):
    ...
    def __init__(self, conn, is_rack_system=False):
        ...
        self.add_background_comm(
            0x0654
        )  # randomly returned occasionally on enabling/disabling channels
        ...
        self._add_status_variable(
            "enabled", self.is_channel_enabled, self.enable_channel
        )
        self._add_settings_variable(
            "output", self.get_output_voltage, self.set_output_voltage
        )
        self._add_settings_variable(
            "output_range", self.get_voltage_range, self.set_voltage_range
        )
        self._add_settings_variable(
        ...
        if self._model.startswith("BPC30"):
            self._add_settings_variable(
                "control_mode", self.get_control_mode, self.set_control_mode
            )
            self._add_settings_variable(
                "output_position", self.get_output_position, self.set_output_position
            )
            self._add_status_variable(
                "status_bits", lambda: self.get_status_bits(channel="all")
            )
    ...
    set_control_mode = KinesisDevice._pzctl_set_control_mode

    # Position control methods (closed loop mode only)
...
TQuadDetectorPIDParams = collections.namedtuple(
    "TQuadDetectorPIDParams", ["p", "i", "d"]
)
TQuadDetectorSetpoint = collections.namedtuple(
    "TQuadDetectorSetpoint", ["xpos", "ypos"]
)
TQuadDetectorReadings = collections.namedtuple(
    "TQuadDetectorReadings", ["xdiff", "ydiff", "sum", "xpos", "ypos"]
)
TQuadDetectorOutputParams = collections.namedtuple(
...
class KinesisQuadDetector(BasicKinesisDevice):
    ...
    def __init__(self, conn, timeout=3.0):
        ...
        self._add_settings_variable(
            "pid_parameters", self.get_pid_parameters, self.set_pid_parameters
        )
        self._add_settings_variable(
            "manual_output", self.get_manual_output, self.set_manual_output
        )
        self._add_status_variable("readings", self.get_readings)
        self._add_settings_variable(
            "operation_mode", self.get_operation_mode, self.set_operation_mode
        )
        self._add_settings_variable(
            "output_parameters", self.get_output_parameters, self.set_output_parameters
        )
    ...
    _p_quad_oper_mode = interface.EnumParameterClass(
        "quad_oper_mode",
        {"monitor": 1, "open_loop": 2, "closed_loop": 3, "auto_loop": 4},
    )
    ...
    _p_quad_route = interface.EnumParameterClass(
        "quad_route", {"sma_only": 1, "sma_hub": 2}
    )
    _p_quad_open_loop_out = interface.EnumParameterClass(
        "quad_open_loop_out", {"zero": 1, "fixed": 2}
    )
    ...
    def get_output_parameters(self):
        ...
        return TQuadDetectorOutputParams(
            lims[0], lims[2], lims[1], lims[3], gains[0], gains[1], par[4], par[5]
        )
    ...
    def set_output_parameters(
        ...
        open_loop_out = (
            current_parameters.open_loop_out if open_loop_out is None else open_loop_out
        )
        data = struct.pack(
            "<hhhhHHhh", xmin, ymin, xmax, ymax, route, open_loop_out, xgain, ygain
        )
